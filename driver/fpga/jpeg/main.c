...
static int _pl_do_encode(void __iomem *reg_base, const struct je_parameter *param)
{
...
static int _pl_set_quantization(void __iomem *reg_base, const struct quantization *q)
{
	u32 index;
	switch (q->type) {
	case e_quantization_type_pl:
		writel(0, reg_base + REG_JPEG_QUANTIZATION_LUT_ADDR);
		break;
	case e_quantization_type_pc:
		writel(64, reg_base + REG_JPEG_QUANTIZATION_LUT_ADDR);
		break;
	case e_quantization_type_sl:
		writel(64 * 2, reg_base + REG_JPEG_QUANTIZATION_LUT_ADDR);
		break;
	case e_quantization_type_sc:
...
static irqreturn_t _irq_error(int irq, void *dev_id)
{
	...
	if (no_resp & (1ul << 1)) {
		printk(KERN_WARNING "IQR[%d]: jpeg encode done interrupt not response\r\n", irq);
		ret = IRQ_HANDLED;
	}

	if (ret == IRQ_HANDLED) {
		dev_priv->result.size = 0;
		dev_priv->result.status = e_je_result_status_error;
...
static irqreturn_t _irq_jpeg_done(int irq, void *dev_id)
{
	...
	dev_priv->result.status = e_je_result_status_succeed;
	dev_priv->interruped = 1;
	wake_up_interruptible(&dev_priv->r_wait);
	return ret;
}
...
static int _open(struct inode *i, struct file *f)
{
	...
	if (_open_flag == 0) {
		...
		dev_warn(_dev_priv->class_dev, "device has been used, open file failed\n");
		ret = -EBUSY;
	}
	return ret;
}
...
static long _unlocked_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
	...
	case IOC_SET_JPEG_QUANTIZATION: {
		...
		ret = copy_from_user(&a, (const void __user *)arg, sizeof(struct quantization));
		if (ret == 0)
			ret = _pl_set_quantization(dev_priv->jpeg_reg_base, &a);
	}
		break;
	case IOC_DO_JPEG_ENCODE: {
		struct je_parameter p;
		ret = copy_from_user(&p, (const void __user *)arg, sizeof(struct je_parameter));
		if (ret == 0)
...
static int _probe(struct platform_device *pdev)
{
	...
	_dev_priv->err_reg_base = devm_ioremap_resource(&pdev->dev, r_mem);
	...
	for (i = 0; i < irq_num; i++) {
		...
		if (h->irq_pf) {
			...
			ret = devm_request_threaded_irq(&pdev->dev, irq, h->irq_pf, h->irq_th_pf, h->flag, dev_name(&pdev->dev), _dev_priv);
			if (ret) {
	...
	_dev_priv->class_dev = device_create(_dev_priv->dev_class, NULL, _devno, NULL, "fpga_jpeg");
